...
2. 安装独立版SysConfig
3. 修改核心路径：前往 SDK 目录下的 `imports.mak`，修改 `TICLANG_ARMCOMPILER`、`SYSCONFIG_TOOL` 等路径为当前电脑的绝对路径。*(注：编译器路径末尾千万不要多写一个 `/bin`，否则会报错找不到文件！)*
4. 在本工程的 `makefile` 第一行 确认 `MSPM0_SDK_INSTALL_DIR` 指向了正确的 SDK 绝对路径
5. 在当前VS Code配置中配置好全局的 `tasks.json` 和 `settings.json` 否则无法使用vscode调用编译 同时全局的配置能便于日后跨工程快捷使用